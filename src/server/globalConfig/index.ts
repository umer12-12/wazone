...
export const getServerGlobalConfig = async () => {
  ...
  // Inject Dynamic Configuration from Database
  try {
    const db = await getServerDB();
    const systemConfigService = new SystemConfigService(db);
    const dbSettings = await systemConfigService.getSettings();

    if (dbSettings) {
      // Branding Injection
      config.wazoneCustomBranding = {
        heroSubtitle: dbSettings['HERO_SUBTITLE'],
        heroTitle: dbSettings['HERO_TITLE'],
        logo: dbSettings['LOGO_URL'],
        name: dbSettings['PRODUCT_NAME'],
      };

      // Feature Toggles (If defined in DB, override ENV)
      if (dbSettings['ENABLE_MARKET'] === 'false') config.enableMarketTrustedClient = false;
    }
  } catch (error) {
    console.error('Failed to inject dynamic Wazone config:', error);
  }
