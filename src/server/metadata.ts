...
export class Meta {
  public generate({
    description = 'Wazone: Your premium AI powerhouse with 100+ specialized tools. Fast, secure, and all-in-one AI Productivity Zone.',
    title,
    image = OG_URL,
    url,
    type = 'website',
    ...
    const formatedTitle = formatTitleLength(title === BRANDING_NAME ? 'Wazone.online' : title, 21);

    const formatedDescription = formatDescLength(description, tags);
    const siteTitle = title === BRANDING_NAME ? 'Wazone.online' : title + ` · Wazone.online`;
    return {
      alternates: {
        canonical:
          canonical ||
